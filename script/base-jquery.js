;(function(){
	...
	function render_task_item(data,i){
		...
		var html = '<div class="task-item" data-id='+i+' >'
				...
				+	'<div>'
				+	'<span><button class=\'action delete\'>Delete</button></span>'
				+	'<span><button class=\'action detail\'>Details</button></span>'
				+	'</div>'
				+ '</div>';
		return $(html);
	}
		...
		$task_detail.on('click',function(e){
			...
			if (target.tagName === 'BUTTON') {
				...
				if (data_detail.date.trim() !== '' && (Date.now() - data_seconds >1) ){
					alert('Alert time can not be current time or before, Please choose another time'); return;
				}
